...
const Experiencia = () => {
    return (
        <section className="mt-14 border-l-2 border-blue-400">
            ...
            {
            experiencia.map(exp => (
                <div className="flex items-center" key={exp.id}>
                    <img loading='lazy' className='w-32 max-md:w-16 rounded-sm' src={exp.img ? `/img/${exp.img}` : "https://cdn-icons-png.flaticon.com/512/189/189792.png"} alt={`Imagen ${exp.nombre}`} />
